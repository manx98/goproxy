...
type DiffZipper struct {
	w   io.Writer
	zw  *zip.Writer
	st  *CreateCheckPointWatcher
	chr cache.Cacher
}

func (z *DiffZipper) Write(data []byte) (n int, err error) {
	relData := make([]byte, len(data)+1)
	relData[0] = BinaryWrite
	copy(relData[1:], data)
	return z.w.Write(relData)
}
...
func (z *DiffZipper) DiffHead(ctx context.Context, tx *bbolt.Tx, oldRoot []byte) error {
	return DiffHead(tx, oldRoot, z.chr, z.st, &DiffOpt{Tx: tx, Ctx: ctx, Callback: func(op OperateType, parent string, dirent *obj.Dirent) error {
		return z.callback(ctx, op, parent, dirent)
	}, FoldDirDiff: false})
}

func NewDiffZipper(chr cache.Cacher, out io.Writer) *DiffZipper {
	zipper := &DiffZipper{
		w:   out,
		chr: chr,
	}
	zipper.zw = zip.NewWriter(zipper)
	zipper.st = &CreateCheckPointWatcher{Writer: zipper}
	return zipper
}
